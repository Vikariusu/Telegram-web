...
class TelegramForm extends Component {
  ...
  componentDidMount() {
    if (localStorage.getItem("telegramMessage")) {
      this.setState(() => ({
        message: localStorage.getItem("telegramMessage"),
      }));
    }
  }
  ...
  onInputChange = (e) => {
    ...
    localStorage.setItem("telegramMessage", value);
    ...
    return (
      <div>
        <form>
          ...
          <textarea onChange={this.onInputChange} name="message" value={this.state.message} />
          <p>
            Characters left:
            <span className={charsLeft > 0 ? "chars-green" : "chars-red"}>
              {charsLeft}
            </span>
          </p>
        </form>
        <StripeCheckout
          ...
          name="telegram"
        >
          <button disabled={charsLeft < 0}>Send Telegram</button>
        </StripeCheckout>
      </div>
    );
  }
